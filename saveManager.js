...
const _saveCarts = new Map();

function _cartSlotKey(cartId, slotIdx) {
  return `cart:${String(cartId)}:${String(slotIdx)}`;
}

function _cartIndexKey(cartId) {
  return `cart:${String(cartId)}:index`;
}

function _cartMetaKey(cartId) {
  return `cart:${String(cartId)}:meta`;
}
...
export function registerSaveCart(cartId, { label = '', maxSaves = 3 } = {}) {
  if (!cartId) return;
  _saveCarts.set(String(cartId), { label, maxSaves: Math.max(1, maxSaves | 0) });
}

export async function setSaveCartLabel(cartId, label) {
  if (!cartId) return false;
  const key = String(cartId);
  const meta = await _readSettingLike(_cartMetaKey(key), { label: label || '' });
  meta.label = label || '';
  await _writeSettingLike(_cartMetaKey(key), meta);
  return true;
}

export async function getSaveCartLabel(cartId) {
  const meta = await _readSettingLike(_cartMetaKey(cartId), null);
  return meta?.label || '';
}
...
export async function saveToCart(cartId, slotName, { label = '' } = {}) {
  if (!cartId || !slotName) return false;
  const key = String(cartId);
  const cart = _saveCarts.get(key) || { maxSaves: 3, label: '' };
  const index = await _readSettingLike(_cartIndexKey(key), []);
  const snapshot = _buildSnapshot(`cart:${key}:${slotName}`);
  snapshot.cart = { cartId: key, slotName, label: label || '', version: CART_VERSION };
  const existing = index.findIndex(entry => entry.slotName === slotName);
  const record = { slotName, label: label || '', timestamp: Date.now() };
  if (existing >= 0) index[existing] = record;
  else index.unshift(record);
  while (index.length > cart.maxSaves) {
    const removed = index.pop();
    if (removed?.slotName) await _deleteSettingLike(_cartSlotKey(key, removed.slotName));
  }
  await _writeSettingLike(_cartSlotKey(key, slotName), snapshot);
  await _writeSettingLike(_cartIndexKey(key), index);
  await _writeSettingLike(_cartMetaKey(key), { label: cart.label || '' });
  return true;
}

export async function loadFromCart(cartId, slotName) {
  if (!cartId || !slotName) return false;
  const data = await _readSettingLike(_cartSlotKey(cartId, slotName), null);
  if (!data) return false;
  _applySnapshot(data);
  return true;
}

export async function listCartSaves(cartId) {
  if (!cartId) return [];
  return await _readSettingLike(_cartIndexKey(cartId), []);
}
...
function _applySnapshot(data) {
  ...
  if (data.story) restoreStorySaveData(data.story);
...
async function _writeSettingLike(key, value) {
  if (window.electron?.writeSetting) {
    try { await window.electron.writeSetting(key, value); return true; } catch { return false; }
  }
  try { localStorage.setItem('setting:' + key, JSON.stringify(value)); return true; } catch { return false; }
}

async function _readSettingLike(key, fallback = null) {
  if (window.electron?.readSetting) {
    try { return await window.electron.readSetting(key); } catch { return fallback; }
  }
  try {
...
async function _deleteSettingLike(key) {
  if (window.electron?.deleteSetting) {
    try { await window.electron.deleteSetting(key); return true; } catch { return false; }
  }
  try { localStorage.removeItem('setting:' + key); return true; } catch { return false; }
}
